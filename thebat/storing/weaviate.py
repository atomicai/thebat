...
from thebat.storing.mask import IDocStore
from thebat.etc.schema import Document, FilterType, Label

# from thebat.tec
from thebat.etc.processor import get_batches_from_generator
from thebat.etc.filter import LogicalFilterClause

# from haystack.document_stores.utils import convert_date_to_rfc3339
from thebat.etc.format import convert_date_to_rfc3339

# from haystack.errors import DocumentStoreError, HaystackError
from thebat.etc.error import DocumentStoreError
...
class IWeaviateDocStore(KeywordDocumentStore):
    ...
    def get_document_by_id(
        ...
        if result:
            ...
            document = self._convert_weaviate_result_to_document(result, return_embedding=True, json_properties=json_properties)
        return document
    ...
    def _get_json_properties(self, index: Optional[str] = None) -> List[str]:
        ...
        for class_item in self.weaviate_client.schema.get()["classes"]:
            if class_item["class"] == index:
                cur_properties = [
                    item["name"] for item in class_item["properties"] if item["description"].startswith("JSON dynamic property")
                ]
    ...
    def query(
        ...
        properties = self._get_current_properties(index)
        if self.similarity == "cosine":
            properties.append("_additional {id, score, certainty, vector}")
        else:
            properties.append("_additional {id, score, distance, vector}")
    ...
    def query_by_embedding_and_bm25(
        self,
        query: str,
        query_emb: np.ndarray,
        alpha: float = 0.5,
